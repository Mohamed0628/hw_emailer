"""Configuration tests for the additional consumer-hardware expansion."""
...
CATALOG = "companies_consumer_hardware_more.yaml"
EXPECTED_COMPANIES = {
    "GoPro",
    "Life360",
    "Owl Labs",
...
def test_catalog_contains_exactly_27_companies():
    payload = _load(CATALOG)
    assert set(_names(payload)) == EXPECTED_COMPANIES
    assert len(_names(payload)) == 27
    assert len(payload.get("greenhouse", [])) == 10
    assert len(payload.get("lever", [])) == 6
    assert len(payload.get("ashby", [])) == 11
...
def test_catalog_does_not_duplicate_existing_company_catalogs():
...
def test_entries_have_public_ats_identifiers():
    payload = _load(CATALOG)
...
def test_expansion_uses_only_lightweight_full_board_sources():
    assert set(_load(CATALOG)) == {"greenhouse", "lever", "ashby"}
